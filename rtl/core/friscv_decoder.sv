...
always_comb begin
    ...
    instr_ex_o = NOP_CTRL;
    instr_ex_o.instr_valid = instr_valid_o;
    instr_ex_o.csr_addr = selected_csr;
    instr_ex_o.csr_is_serializing = is_serializing_csr(selected_csr);
    instr_ex_o.csr_is_counter = is_counter_csr(selected_csr);
    rs1_sel_o = 5'b0;
    rs2_sel_o = 5'b0;
    rd_sel_o  = 5'b0;
    ...
    case (ir_i.r.opcode)
        LOAD: begin
            ...
            instr_ex_o.alu_op = ADD_OP;
            instr_ex_o.mem_instr_sel = MEM_INSTR_LOAD;
            instr_ex_o.load_store_width = mem_width_e'(ir_i.r.funct3);
            ...
            if (EnableIsaE && (ir_i.r.rs1[4] || ir_i.r.rd[4])) illegal_inst_o = 1'b1;
        end
        ...
        MISC_MEM: begin
            case (ir_i.r.funct3)
                ...
                3'b001: begin  // FENCE.I
                    ...
                    instr_ex_o.branch_jal_sel = BRANCH_INSTR;
                    instr_ex_o.branch_cond = COND_EQ;
                    instr_ex_o.a_bus_sel = RS1;    // Branch address = x0 + next_pc
                    instr_ex_o.b_bus_sel = IMM;
                    imm_sel_o = NEXT_PC;
                    instr_ex_o.alu_op = ADD_OP;
                    instr_ex_o.mem_instr_sel = MEM_INSTR_NONE;
                end
        ...
        STORE: begin
            ...
            if (ir_i.r.funct3 == 3'b011 || ir_i.r.funct3 >= 3'b100) illegal_inst_o = 1'b1;

            imm_sel_o = S_TYPE;
            rs1_sel_o = ir_i.r.rs1;
            rs2_sel_o = ir_i.r.rs2;

            if (EnableIsaE && (ir_i.r.rs1[4] || ir_i.r.rs2[4])) illegal_inst_o = 1'b1;
        end
            ...
            if (EnableIsaA) begin
                case (ir_i.r.funct3)
                    3'b010: begin  // RV32A Standard Extension instructions
                        ...
                        instr_ex_o.mem_instr_sel = MEM_INSTR_LOAD;
                        instr_ex_o.load_store_width = WIDTH_I32;
                        instr_ex_o.alu_op = ADD_OP;
                        instr_ex_o.a_bus_sel = RS1;
                        instr_ex_o.b_bus_sel = IMM;
                        ...
                        case (ir_i.r.funct7[6:2])
                            5'b00011: begin                            // SC.W
                                ...
                                instr_ex_o.wb_data_sel = WB_DATA_SEL_SC_RES;
                            end
                            5'b00010: begin                            // LR.W
                                instr_ex_o.reserve = 1'b1;
                                if (ir_i.r.rs2 != 5'b0) illegal_inst_o = 1'b1;
                            end
                            5'b00001: instr_ex_o.amo_op = AMO_SWAP;  // AMOSWAP.W
                            5'b00000: instr_ex_o.amo_op = AMO_ADD;   // AMOADD.W
                            5'b00100: instr_ex_o.amo_op = AMO_XOR;   // AMOXOR.W
                            5'b01100: instr_ex_o.amo_op = AMO_AND;   // AMOAND.W
                            5'b01000: instr_ex_o.amo_op = AMO_OR;    // AMOOR.W
                            5'b10000: instr_ex_o.amo_op = AMO_MIN;   // AMOMIN.W
                            5'b10100: instr_ex_o.amo_op = AMO_MAX;   // AMOMAX.W
                            5'b11000: instr_ex_o.amo_op = AMO_MINU;  // AMOMINU.W
                            5'b11100: instr_ex_o.amo_op = AMO_MAXU;  // AMOMAXU.W
                            default:  illegal_inst_o = 1'b1;
                        endcase
                    end
                    default: illegal_inst_o = 1'b1;
                ...
                if (EnableIsaE && (ir_i.r.rs1[4] || ir_i.r.rs2[4] || ir_i.r.rd[4])) illegal_inst_o = 1'b1;
            end else begin
                illegal_inst_o = 1'b1;
            end
        end
        ...
        OP: begin  // rd <- rs1 <op> rs2
            ...
            instr_ex_o.a_bus_sel = RS1;
            instr_ex_o.b_bus_sel = RS2;
            instr_ex_o.mem_instr_sel = MEM_INSTR_NONE;
            instr_ex_o.wb_data_sel   = WB_DATA_SEL_ALU;
            ...
            case (ir_i.r.funct3)
                3'b000:
                    ...
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = MUL_OP;    // MUL
                    else illegal_inst_o = 1'b1;
                3'b001:
                    if      (ir_i.r.funct7 == 7'b0000000) instr_ex_o.alu_op = SLL_OP;                  // SLL
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = MULH_OP;   // MULH
                    else illegal_inst_o = 1'b1;
                3'b010:
                    if      (ir_i.r.funct7 == 7'b0000000) instr_ex_o.alu_op = SLT_OP;                  // SLT
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = MULHSU_OP; // MULHSU
                    else illegal_inst_o = 1'b1;
                3'b011:
                    if      (ir_i.r.funct7 == 7'b0000000) instr_ex_o.alu_op = SLTU_OP;                 // SLTU
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = MULHU_OP;  // MULHU
                    else illegal_inst_o = 1'b1;
                3'b100:
                    if      (ir_i.r.funct7 == 7'b0000000) instr_ex_o.alu_op = XOR_OP;                  // XOR
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = DIV_OP;    // DIV
                    else illegal_inst_o = 1'b1;
                3'b101:
                    if      (ir_i.r.funct7 == 7'b0000000) instr_ex_o.alu_op = SRL_OP;                  // SRL
                    else if (ir_i.r.funct7 == 7'b0100000) instr_ex_o.alu_op = SRA_OP;                  // SRA
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = DIVU_OP;   // DIVU
                    else illegal_inst_o = 1'b1;
                3'b110:
                    if      (ir_i.r.funct7 == 7'b0000000) instr_ex_o.alu_op = OR_OP;                   // OR
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = REM_OP;    // REM
                    else illegal_inst_o = 1'b1;
                3'b111:
                    if      (ir_i.r.funct7 == 7'b0000000) instr_ex_o.alu_op = AND_OP;                  // AND
                    else if (ir_i.r.funct7 == 7'b0000001 && EnableIsaM) instr_ex_o.alu_op = REMU_OP;   // REMU
                    else illegal_inst_o = 1'b1;
                default: illegal_inst_o = 1'b1;
            endcase
            ...
            if ((ir_i.r.funct3 == 3'b001 ||
                 ...
                 ir_i.r.funct3 == 3'b111) && (ir_i.r.funct7 != 7'b0 && ir_i.r.funct7 != 7'b0000001))
                illegal_inst_o = 1'b1;
        ...
        OP_IMM: begin  // rd <- rs1 <op> imm
            ...
            case (ir_i.r.funct3)
                ...
                3'b100: instr_ex_o.alu_op = XOR_OP;       // XORI
                3'b110: instr_ex_o.alu_op = OR_OP;        // ORI
                3'b111: instr_ex_o.alu_op = AND_OP;       // ANDI
                // I2-type shift instructions (with immediate shamt)
                3'b001: begin                                     // SLLI
                    imm_sel_o = I2_TYPE;
                    instr_ex_o.alu_op = SLL_OP;
        ...
        AUIPC: begin  // rd <- PC + (imm << 12)
            ...
            // Uses rd
            if (EnableIsaE && ir_i.r.rd[4]) illegal_inst_o = 1'b1;
        end
        ...
        BRANCH: begin  // pc <- pc + (imm << 1) if <branch_cond>(rs1, rs2)
            ...
            case (ir_i.r.funct3)
                ...
                3'b101:  instr_ex_o.branch_cond = COND_GE;
                3'b110:  instr_ex_o.branch_cond = COND_LTU;
                3'b111:  instr_ex_o.branch_cond = COND_GEU;
                default: illegal_inst_o = 1'b1;
            endcase
        ...
        JALR: begin
            instr_ex_o.branch_jal_sel = JAL_INSTR;
            instr_ex_o.jalr_target = 1'b1;
            instr_ex_o.a_bus_sel = RS1;
            instr_ex_o.b_bus_sel = IMM;
            instr_ex_o.alu_op = ADD_OP;
            instr_ex_o.mem_instr_sel = MEM_INSTR_NONE;
            instr_ex_o.wb_data_sel = WB_DATA_SEL_PC_PLUS_4;
            ...
            if (ir_i.r.funct3 == 3'b0) begin  // Non-CSR SYSTEM instructions
                case (ir_i.r.funct7)
                    7'b0001001: begin  // SFENCE.VMA
                        ...
                        else if (mode_i == S_MODE && tvm_i) illegal_inst_o = 1'b1;    // S-mode with TVM=1 cannot execute SFENCE.VMA.
                        else begin
                            instr_ex_o.sfence_vma = 1'b1;
                            // Flush pipeline and jump to PC+4 to refetch with a clean TLB.
                            instr_ex_o.branch_jal_sel = BRANCH_INSTR;
                            instr_ex_o.branch_cond = COND_ALWAYS;
                            instr_ex_o.a_bus_sel = ZERO_A;
                            instr_ex_o.b_bus_sel = IMM;
                            imm_sel_o = NEXT_PC;
                            instr_ex_o.alu_op = ADD_OP;
                            instr_ex_o.mem_instr_sel = MEM_INSTR_NONE;
                ...
                instr_ex_o.csr_op      = 1'b1;
                instr_ex_o.wb_data_sel = WB_DATA_SEL_CSR;
                rs1_sel_o = ir_i.r.rs1;
                rd_sel_o  = ir_i.r.rd;
                ...
                case (ir_i.r.funct3)
                    3'b001: begin  //  CSRRW
                        instr_ex_o.a_bus_sel = RS1;
                        instr_ex_o.b_bus_sel = IMM;
                        instr_ex_o.alu_op    = OR_OP;
                        imm_sel_o = ZERO;
                    end
                    3'b010: begin  // CSRRS
                        instr_ex_o.a_bus_sel = RS1;
                        instr_ex_o.b_bus_sel = CSR;
                        instr_ex_o.alu_op    = OR_OP;
                    end
                    3'b011: begin  // CSRRC
                        instr_ex_o.a_bus_sel   = RS1;
                        instr_ex_o.invert_op_a = 1'b1;
                        instr_ex_o.b_bus_sel   = CSR;
                        instr_ex_o.alu_op      = AND_OP;
                    end
                    3'b101: begin  // CSRRWI
                        instr_ex_o.a_bus_sel = RS1_SEL;
                        instr_ex_o.b_bus_sel = IMM;
                        instr_ex_o.alu_op    = OR_OP;
                        imm_sel_o = ZERO;
                    end
    ...
    if (illegal_inst_o) instr_ex_o = NOP_CTRL;
end
